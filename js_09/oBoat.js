...
var oBoat = {
	...
	getSeatTypeText : function(boatRecord, boatVariantIndex) {
		...
		for (var i = 0; i < bTypeSeats.length; i++) {
			var withCox = (bTypeCoxed[i].localeCompare("COXED") == 0) ? " mit"
					: "";
			if ($_efaTypes && $_efaTypes["NUMSEATS"] && $_efaTypes["NUMSEATS"][bTypeSeats[i]])
	...
	updateBoatStatusOnSession : function(tx) {
		...
		// update with the server generated keys. This will also adjust a fixed EntryId 
		if (tx.resultMessage)
		...
		for (boatStatusId in bPanel.allBoatsStatus) {
			...
			var sameBoat = (boatStatusId.localeCompare(tripRecord.BoatId) == 0); // known boat
			if (sameBoat)
				relatedBoatStatus = boatStatus;
		}
		// If the boat is unknown and the trip is ending, find the temporary boatstatus by matching the sessions EntryId.
		if (!relatedBoatStatus)
			for (boatStatusId in bPanel.allBoatsStatus) {
				if (oBoatstatus.isOnTheWater(boatStatus)) {
					// the efaWeb-logbook is always the current one, not multiple year, soe EntryId is unique
					var session = cLists.lists.efaWeb_logbook[cLists.indices.efaWeb_logbook_nids[boatStatus.EntryNo]];
					var sameEntryId = (parseInt(session.EntryId) == parseInt(tripRecord.EntryId)); // same session
					if (sameEntryId)
						relatedBoatStatus = boatStatus;
				}
			}
		
		var boatId = (relatedBoatStatus) ? relatedBoatStatus.BoatId : tripRecord["BoatId"];
		if (!boatId && !tripRecord["BoatName"])
			return;
		var unknownBoat = (!tripRecord["BoatId"]) || !relatedBoatStatus || (relatedBoatStatus.UnknownBoat.localeCompare("true") == 0);
		var boat = (unknownBoat) ? false : cLists.lists.efaWeb_boats[cLists.indices.efaWeb_boats_guids[boatId]];
		...
		if (relatedBoatStatus) {
			...
			record["BoatText"] = (boat) ? boat.Name : relatedBoatStatus["BoatText"];
			record["BaseStatus"] = relatedBoatStatus["BaseStatus"];
			record["ecrid"] = relatedBoatStatus["ecrid"];
		} else {
			record["BoatId"] = (unknownBoat) ? cToolbox.generateGUID() : boatId; // == boat.Id
			record["BoatText"] = (unknownBoat) ? tripRecord["BoatName"] : boat.Name;
			record["BaseStatus"] = "AVAILABLE";  // for both an unknown boat and a boat with missing boatstatus record
		...
		if (tripRecord["Open"].localeCompare("false") == 0) {
			...
			var destination = (tripRecord["DestinationId"]) ? 
					cLists.lists.efaWeb_destinations[cLists.indices.efaWeb_destinations_guids[tripRecord["DestinationId"]]]["Name"]
					: tripRecord["DestinationName"];
			var onTheWaterComment = "Unterwegs nach " + destination + " seit "
					+ tripRecord["Date"] + " um " + tripRecord["StartTime"]
					+ " mit " + tripRecord["AllCrewNames"] + " als Fahrt Nr. " + tripRecord["EntryId"]
					+ " im Fahrtenbuch " + $_logbookname;
