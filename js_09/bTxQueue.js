...
var bTxQueue = {
	...
	apiMinVersion : 1,   
	apiMaxVersion : $_max_api_version_efaWeb,   
	apiVersion : 1,   
	...
	buildContainer : function(txs) {
		...
		// nop response.
		this.apiVersion = Math.min($_max_api_version_server, $_max_api_version_efaWeb);
		...
		var base64efa = base64.replace(/\//g, '-').replace(/\+/g, '*').replace(
				/\=/g, '_');
		console.log("V" + this.apiVersion + " container built: " + txs.length + " transactions in " + base64efa.length + " base64 characters");
		return base64efa;
	},
	 ...
	 * callback procedure for container success (bTxQueue.handleResponse) and
	 * failure (bTxHandler.handleContainerError). Then sends the request.
	 */
	...
	handleResponse : function(txrc) {
		...
		for (txr of txrc.txrs) {
			...
			if (tx) {
				...
				console.log("WARNING: transaction #" + tc + " of " + txrc.txrs.length + " is undefined @bTxQueue.handleResponse.");
			}
			tc ++;
		}
	},
