...
var oSession = {
	...
	buttonEndSession : '<div class="w3-row"><div class="w3-col l1">'
			+ '<p class="formbutton" id="do-getForm_endSession_[EntryId]">Fahrt Beenden</p>'
			+ '</div></div>',
	...
	formatSession : function(session, full) {
		...
		var html = "<tr><td>" + session["EntryId"] + "</td><td>" + startAndEnd
	...
	getDatasheet : function(session) {
		...
		s += "</tbody></table>";
		var endButton = (session.Open.localeCompare("true") == 0) ? oSession.buttonEndSession
				.replace("[EntryId]", "" + session.EntryId)
				: "";
		return s + endButton;
	},
	...
	resolveSessionData : function(record) {
		// resolve boat name
		var boatVariantName = bForm.inputs["BoatId"];
		var boatId = cLists.names.efaweb_virtual_boatVariants_names[bForm.inputs["BoatId"]];
		var boat = false;
		if (boatId) {
		...
		if (bForm.inputs["CoxId"] && (bForm.inputs["CoxId"].length > 1)) {
			...
			var coxId = cLists.names.efaWeb_persons_names[bForm.inputs["CoxId"]];
			if (!coxId)
		...
		for (var i = 1; i <= 8; i++) {
			if (bForm.inputs["Crew" + i + "Id"]
				...
				var crewId = cLists.names.efaWeb_persons_names[bForm.inputs["Crew"
						+ i + "Id"]];
				if (!crewId)
					// if the UUID could not be resolved, use the name.
		...
		var destinationId = cLists.names.efaWeb_destinations_names[bForm.inputs["DestinationId"]];
		if (destinationId) {
