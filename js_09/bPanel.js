...
var bPanel = {
	...
	_boatsToShow : function(available) {
		...
		for (boatId in this.allBoatsStatus) {
			...
			var isUnknownBoat = (boatstatus && boatstatus["UnknownBoat"].localeCompare("true") == 0);
	...
	listAvailableBoats : function() {
		...
		boatsToShow.forEach(function(boat) {
			cntOrStatusText = oBoat.getSeatTypeText(boat);
			if (cntOrStatusText) {
	...
	showList : function(type, count) {
		...
		if (type.localeCompare("logbook") == 0) {
			...
			header += (($_logbook_allowance_all) ? "Fahrtenbuch " + $_logbookname : "Eigene Fahrten in diesem Jahr");
