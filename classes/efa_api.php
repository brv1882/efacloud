...
class Efa_api
{
            ...
            int $api_version = 1)
    {
        if ($this->summary_logbook_filter($tablename, $record) !== false)
            return "502;records of a virtual summary logbook cannot be modified.";
        ...
        if ($api_version < 3) {
            ...
            if (! is_array($pre_modification_check))
                return "502;" . $pre_modification_check;
        ...
        if (strlen($result) == 0) {
            ...
            if ($key_was_modified) {
                ...
                if ($this->debug_on)
                    file_put_contents(Tx_handler::$api_debug_log_path, 
                            date("Y-m-d H:i:s") . ": api_$mode_str: completed. Ecrid '" . $record["ecrid"] .
                                     "' \n", FILE_APPEND);
            ...
            if ($this->debug_on)
                file_put_contents(Tx_handler::$api_debug_log_path, 
                        date("Y-m-d H:i:s") . ": api_$mode_str: Failed. Reason: '" . $result . "' \n", 
                        FILE_APPEND);
    ...
    public function api_keyfixing (array $client_verified, String $tablename, array $fixed_record_reference)
    {
        if ($this->debug_on)
            file_put_contents(Tx_handler::$api_debug_log_path, 
                    date("Y-m-d H:i:s") . ": starting api_keyfixing\n", FILE_APPEND);
        
        if (! array_key_exists($tablename, Efa_tables::$efa_autoincrement_fields)) {
            // 502 => "Transaction failed.", if the key must not be fixed.
            if ($this->debug_on)
                file_put_contents(Tx_handler::$api_debug_log_path, 
            ...
            bool $keys_only)
    {
        ...
        foreach ($tnames as $tname) {
            ...
            if ($isLogbooktable) {
                ...
                $condition .= ($summary_logbook_filter !== false) ? ",LIKE" : ",=";
            ...
            while (($records !== false) && (count($records) > 0)) {
                ...
                foreach ($records as $record) {
                    ...
                    if (count($record) > 0) {
                        ...
                        // Modify the entryId for logbook records of a virtual summary logbook.
                        if ($isSummaryLogbook) {
                            if (isset($record["EntryId"]) && isset($logbooksequence[$record["Logbookname"]])) {
                                $record["EntryId"] = strval($record["EntryId"]) .
                                         strval($logbooksequence[$record["Logbookname"]]);
                            }
                        }
     ...
     * @return array all names found, an empty array on no match.
     */
    private function list_logbooks_for_summary (String $filter_like)
    {
        $res = $this->socket->query(
                "SELECT DISTINCT `Logbookname` FROM `efa2logbook` WHERE `Logbookname` LIKE '" . $filter_like .
                         "'");
        $logbooksequence = [];
        $l = 1;
        if (isset($res->num_rows) && (intval($res->num_rows) > 0)) {
            $row = $res->fetch_row();
            while ($row) {
                $logbooksequence[$row[0]] = $l;
                $l ++;
                $row = $res->fetch_row();
            }
        }
        return $logbooksequence;
    }
