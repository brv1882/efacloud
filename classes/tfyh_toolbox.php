...
class Tfyh_toolbox
{
     ...
     *            any array
     * @param String $indent
     *            the current indentation for recursive calls, never to be used when calling from
     *            outside.
    ...
    public static function array_to_html (array $a, String $indent = "")
    {
        ...
        foreach ($a as $key => $value) {
            if (is_array($value)) {
                ...
                $disp = (is_null($value)) ? "<span style=\"color:#008;\">NULL</span>" : ((is_bool(
                        $value)) ? ("<span style=\"color:#808;\">" . (($value) ? "true" : "false") .
                         "</span>") : ((is_string($value)) ? "<span style=\"color:#088;\">\"" .
                         $value . "\"</span>" : $value));
    ...
    /**
     * Read a simplified csv-String into an array of rows, each row becoming a named array with the
     * names being the first line entries. CSV-format must be with text delimiter = " and separator
     * = ;. There must not be any space character left or right of the delimiter. First line entries
     * must not contain line breaks. Lines ending with unquoted " \" will be joined with the
     * following line. The file is preferrably encoded in UTF-8, but ISO-8859-1 should also work due
            ...
            if (strlen(trim($line)) > 0) { // ignore empty lines and lines with only blanks
                ...
                if (is_null($header))
                    ...
                    if ($cnt_quotes % 2 == 0) {
                        ...
                        if (strcmp(mb_substr($completed_line, mb_strlen($completed_line) - 2), 
                                " \\") == 0) {
    ...
    public function load_warning (String $event_directory, String $source_file)
    {
        ...
        if (strlen($event_directory) > 0) {
            ...
            $log_text .= "-------------------- Timestamps ------------------\n" . $event_directory .
                     " timestamps:\n";
            for ($i = - 3; $i < 4; $i ++)
                $log_text .= strval($event_pointer + $i) . ";" .
                         date("Y-m-d H:i:s", 
                                filectime(
                                        "../log/" . $event_directory . strval($event_pointer + $i))) .
                         "\n";
        }
