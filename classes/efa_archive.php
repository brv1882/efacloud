...
class Efa_archive
{
    ...
    public function __construct (Tfyh_toolbox $toolbox, Tfyh_socket $socket, int $appUserID)
    {
        ...
        foreach (self::$archive_settings as $for_table => $archive_setting) {
            // the
            $archive_min_age_days = (! isset($cfg[$archive_setting["ListParam"]])) ? Efa_tables::$forever_days : ((intval(
                    $cfg[$archive_setting["ListParam"]]) > 180) ? intval($cfg[$archive_setting["ListParam"]]) : 180);
            self::$archive_settings[$for_table]["MinAgeDays"] = $archive_min_age_days;
    ...
    private function move_to_archive (array $named_row_to_move, String $table_name)
    {
        ...
        $modification = self::$archive_settings["$table_name"]["Origin"];
            ...
            int $last_modified_secs, String $change_count)
    {
        ...
        $last_modification = self::$archive_settings[$tablename]["Origin"];
        ...
        $last_modification = self::$archive_settings[$tablename]["Origin"];
        $nominal_stub = Efa_tables::register_modification($nominal_stub, $last_modified_secs, $change_count, 
                $last_modification);
    ...
    public function autocorrect_archive_stubs (String $tablename, int $min_age_days_for_check)
    {
        ...
        $dublets = 0;
        $ecrids_handled = [];
        $modification = self::$archive_settings[$tablename]["Origin"];
                    ...
                    if (isset($ecrids_handled[$archived_record["ecrid"]])) {
                        ...
                        if ($current_stub === false) {
                            ...
                            $nominal_stub = Efa_tables::register_modification($nominal_stub, $now, 
                                    $change_count_current, $modification);
                            // echo $archive_id . "<br>nominal stub<br>";
                            // var_dump($nominal_stub);
        ...
        if ($corrected == 0)
            $result = "checked/total count: " . ($checked - $skipped) . "/" . $checked .
                     ", no corrections needed";
        else
            $result = "checked: $checked, corrected: $corrected, correction failed: $failed," .
                     " dublts in archive: $dublets, no check or no reference needed: $skipped.";
        return $result;
    }
    ...
    private function restore_from_archive (int $archive_id, String $tablename, array $archived_record)
    {
        ...
        if ($stub === false) {
            ...
            $archived_record = Efa_tables::register_modification($archived_record, time(), 
    ...
    public function restore_form_archive (String $tablename, int $archived_less_than_days_ago)
    {
        try {
                ...
                if (! is_null($archived_record) && is_array($archived_record)) {
                    ...
                    if ($stub !== false) {
                        ...
                        if (strlen($update_result) == 0) {
                            ...
                            $failed ++;
                            $failure_log .= "Der Archivdatensatz #" . $archive_id .
                                     " konnte nicht wiederhergestellt werden. Grund: " . $update_result .
                                     "<br>";
                        }
                    } else {
                        ...
                        if (is_numeric($insert_result)) {
                            ...
                            $failed ++;
                            $failure_log .= "Der Archivdatensatz #" . $archive_id .
                                     " konnte nicht wiederhergestellt werden. Grund: " . $insert_result .
                                     "<br>";
                        }
                    }
                } else {
                    $failed ++;
                    $failure_log .= "Der Archivdatensatz #" . $archive_id .
                             " konnte nicht wiederhergestellt werden. Grund: Die Dekodierung des Datensatzes ist gescheitert.<br>";
                }
            }
            ...
            return "Die Wiederherstellung für Datensätze aus $tablename, die vor weniger als $archived_less_than_days_ago " .
                     "Tagen archiviert wurden, hat leider zu folgendem Fehler geführt: " . $e->getMessage() .
                     " Bitte wende Dich mit der Angabe des Fehlers an den Support.";
        }
    }
