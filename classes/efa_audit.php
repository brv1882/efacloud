...
class Efa_audit
{
    ...
    public function data_integrity_audit (bool $html)
    {
        ...
        // =========================================================
        $audit_result = "<li><b>Liste der Objekt IDs" .
                 "<sup class='eventitem' id='showhelptext_UUIDecrid'>&#9432;</sup>" .
                 " in der Datenbank</b></li>\n<ul>";
        $audit_log = date("Y-m-d H:i:s") . " +0 Starting data base integrity audit.\n";
        $all_ids_count = 0;
        ...
        for ($list_index = 0; $list_index < count($list_definitions); $list_index ++) {
            ...
            if ($ids_count > 0)
                $audit_result .= "<li>$ids_count Objekt IDs in $table_name.</li>\n";
            $all_ids_count += $ids_count;
        }
        $duration = time() - $last_step;
        $last_step = time();
        $audit_result .= "<li>$all_ids_count Objekt IDs in Summe.</li>\n";
        $audit_result .= "<li>Dauer: $duration Sekunden.</li>";
        $audit_result .= "</ul>";
        
        // continue with compilation of all UUID references
        // ================================================
        $audit_result .= "<li><b>Liste der Referenzen auf Objekt IDs" .
                 "<sup class='eventitem' id='showhelptext_UUIDecrid'>&#9432;</sup>" .
                 " in der Datenbank</b></li>\n<ul>";
        $all_id_refs_count = 0;
        ...
        for ($list_index = 0; $list_index < count($list_definitions); $list_index ++) {
            ...
            if ($id_refs_count > 0)
                $audit_result .= "<li>$id_refs_count Referenzen auf Objekt IDs in $table_name</li>\n";
        ...
        $duration = time() - $last_step;
        $last_step = time();
        $audit_result .= "<li>In Summe $all_id_refs_count Referenzen auf Objekt IDs in der Datenbank.</li>\n";
        $audit_result .= "<li>Dauer: $duration Sekunden.</li>";
        $audit_result .= "</ul>";
        $audit_log .= date("Y-m-d H:i:s") . " +" . $duration . " References collected\n";
        
        // continue with check for corrupt data
        // ====================================
        $audit_result .= "<li><b>Liste der korrupten Datensätze" .
                 "<sup class='eventitem' id='showhelptext_AuditKorrupteDatensaetze'>&#9432;</sup>" .
                 "</b><br>Datensätze, bei denen die Angabe " .
                 "der letzten Änderung fehlt werden automatisch korrigiert, pro Wartungslauf und Tabelle " .
                 "50 Stück maximal.</li>\n<ul>";
        $all_id_refs_count = 0;
        ...
        for ($list_index = 0; $list_index < count($list_definitions); $list_index ++) {
            ...
            foreach ($lists["corrupt"][$list_id]->get_rows() as $row) {
                ...
                if (isset($row[$lastModified_index]) && strlen($row[$lastModified_index]) < 3)
                    $cause .= "Angabe zum Zeitpunkt der letzten Änderung ist ungültig, ";
                if (! isset($row[$changeCount_index]) || is_null($row[$changeCount_index]) ||
                         strlen($row[$changeCount_index]) == 0)
                    $cause .= "Änderungszähler fehlt, ";
                if (isset($row[$changeCount_index]) && intval($row[$changeCount_index]) == 0)
                    $cause .= "Änderungszähler ungültig, ";
                if (strlen($cause) == 0)
                    $cause = "unbestimmter Fehler, bitte Datensatz prüfen.";
                // get full record
                ...
                $empty_notification = (Efa_record::is_content_empty($table_name, $full_record)) ? "(inhaltlich leer) " : "";
                $corrupt_records_list .= "<li>$cause in Datensatz mit der ID '" . $row[$ecrid_index] .
                         "' $empty_notification<a class='eventitem' id='viewrecord_" . $table_name . "_" .
                         $row[$ecrid_index] . "'>ansehen</a></li>\n";
                // collect key of referencing record
                $corrupt_records_cnt ++;
            }
        ...
        $duration = time() - $last_step;
        $last_step = time();
        $dauer_li = "<li>Dauer: $duration Sekunden.</li>";
        if ($corrupt_records_cnt_all > 0)
            $audit_result .= $corrupt_records_list_all . $dauer_li . "</ul>";
        else
            $audit_result .= "<li>Keine.</li>$dauer_li</ul>";
        $audit_log .= date("Y-m-d H:i:s") . " +" . $duration . " Corrupt records fixed.\n";
        
        // continue with duplicate warnings and unique assertions
        // ======================================================
        $audit_result .= "<li><b>Dublettencheck</b>" .
        ...
        $audit_result .= "<li>Dauer: $duration Sekunden.</li>";
        $audit_result .= "</ul>";
        $audit_log .= date("Y-m-d H:i:s") . " +" . $duration . " Duplicates checked.\n";
        
        // continue with non-empty checks
        // ==============================
        $audit_result .= "<li><b>Fehlende Angaben" .
                 "<sup class='eventitem' id='showhelptext_AuditKorrupteDatensaetze'>&#9432;</sup>" .
                 "</b></li>\n<ul>";
        ...
        for ($list_index = 0; $list_index < count($list_definitions); $list_index ++) {
            ...
            foreach ($lists["nonempty"][$list_id]->get_rows() as $row) {
                ...
                if (strlen($defaulting) > 0) {
                    $audit_result .= "<li>die notwendigen Angaben '" . $defaulting . "' fehlen im Datensatz: " .
                             $recordstr . "'.</i></li>\n";
                }
                if (strlen($missing) > 0) {
        ...
        $audit_result .= "<li>Dauer: $duration Sekunden.</li>";
        $audit_result .= "</ul>";
        $audit_log .= date("Y-m-d H:i:s") . " +" . $duration . " Missing fields checked.\n";
        
        // continue with virtual fields check and correction
        // =================================================
        $audit_result .= "<li><b>Virtuelle Datenfelder" .
        ...
        for ($list_index = 0; $list_index < count($list_definitions); $list_index ++) {
            ...
            foreach ($lists["virtual"][$list_id]->get_rows() as $row) {
                ...
                if ($record_with_correct_vf !== false) {
                    $audit_result .= "<br>Korrigiere virtuelle Felder für Datensatz mit ecrid '" .
            ...
            if (($corrected > 0) || ($failed > 0))
                $audit_result .= " überprüft: $checked, korrigiert: $corrected, Fehler: $failed.</li>\n";
            else
                $audit_result .= " ok.</li>\n";
        }
        $duration = time() - $last_step;
        ...
        $audit_result .= "<li>Dauer: $duration Sekunden.</li>";
        $audit_result .= "</ul>";
        $audit_log .= date("Y-m-d H:i:s") . " +" . $duration . " Virtual fields corrected.\n";
        ...
        foreach ($uuid_names as $uuid => $name)
            if ((! isset($uuid_refs[$uuid]) || (count($uuid_refs[$uuid]) == 0)) &&
                ...
                $unused_count ++;
        $audit_result .= "<li><b>Die folgenden $unused_count der in Summe $all_ids_count Objekt IDs werden nicht (mehr) verwendet:</b></li>\n<ul>";
        foreach ($uuid_names as $uuid => $name) {
            if (! isset($uuid_refs[$uuid]) || (count($uuid_refs[$uuid]) == 0)) {
        ...
        $audit_result .= "<li>Dauer: $duration Sekunden.</li>";
        $audit_result .= "</ul>";
        $audit_log .= date("Y-m-d H:i:s") . " +" . $duration . " Completed.\n";
    ...
    public function period_correctness_audit ()
    {
        ...
        $audit_res = "<li><b>Fahrten, die nicht zur Fahrtenbuchperiode passen.</b></li><ul>";
        include_once "../classes/tfyh_list.php";
        ...
        foreach ($rows as $row) {
            ...
            if (! $is_delete) {
                ...
                if ($logbook_period["book_matched"] === false)
                    $audit_res .= "<li>Das Fahrtenbuch $logbook_name (Fahrtnummer: '$entry_id') ist nicht bekannt.</li>";
                else {
                    ...
                    if (($entry_start < $logbook_start) || ($entry_start > $logbook_end) ||
                        ...
                        $time_trip = "<li>Die Fahrt #$entry_id mit Beginn " . date("d.m.Y", $entry_start);
                        $time_trip .= " und Ende " . date("d.m.Y", $entry_end);
                        $error_message = $time_trip .
        ...
        $audit_res .= "<li><b>Vereinsarbeit, die nicht zur Periode des Vereinsarbeitsbuchs passen.</b></li><ul>";
        ...
        foreach ($rows as $row) {
            ...
            $clubworkbook_period = $efa_config->get_book_period($clubworkbook_name, true);
            $is_delete = (strcasecmp($row[$index_lastmodification], "delete") == 0);
            if (! $is_delete) {
                if ($clubworkbook_period["book_matched"] === false)
                    $audit_res .= "<li>Das Vereinsarbeitsbuch $clubworkbook_name ist nicht bekannt.</li>";
                else {
                    ...
                    if (($work_date < $workbook_start) || ($work_date > $workbook_end)) {
                        $time_work = "<li>Die Vereinsarbeit '$description' am Tag " .
                                 date("d.m.Y", $entry_start);
                        $error_message = $time_work .
                                 " liegt nicht im gültigen Zeitraum für das Vereinsarbeitsbuch '$clubworkbook_name' (" .
                                 date("d.m.Y", $workbook_start) . " - " . date("d.m.Y", $workbook_end) . ")";
                        $error_message .= " <a class='eventitem' id='viewrecord_" . $list->get_table_name() .
                                 "_" . $row[$index_ecrid] . "'>ansehen</a>";
                        if ($user_is_admin)
